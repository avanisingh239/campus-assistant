...
> **As built:** all four of this section's screens (Admin Dashboard, Submit Class Update, Submit Society Update, Submission History) are implemented as one consolidated route, `/admin/dashboard`, that adapts to the signed-in admin's own scope — not four separate routes. Same supersession pattern as `/login` and `/student/ingest` above. `/admin/submit/class`, `/admin/submit/society`, and `/admin/history` briefly stayed as untouched placeholders, then were deleted outright (along with their admin-nav links) once a follow-up pass found nothing pointed at them anymore — these route names no longer exist in the codebase at all. Course Code, Teacher Name, and Venue/Room (named in the field lists above) aren't collected — `announcements` has no matching column for any of the three, same constraint the AI extraction schema was already built around (see `docs/ai-contracts.md` §3). See CLAUDE.md's §Admin Dashboard.
