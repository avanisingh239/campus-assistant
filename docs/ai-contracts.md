...
**Rate limits:** Gemini's free tier caps requests at roughly 10/minute. `lib/ai/extract.ts` retries a 429 or 503 response up to twice with exponential backoff (1s, 2s) before throwing a `RateLimitError` whose message is meant to be shown to the user directly (see `app/student/ingest/result-panel.tsx`'s Failed result state). This is also why `app/student/ingest/ingest-screen.tsx` caps a WhatsApp `.txt` upload at 25 messages — one Gemini call per message, sequentially, would otherwise mean an increasingly backed-off request queue for a large export.
...
> **As built:** `app/student/ingest/` (see CLAUDE.md's §Ingestion page) is the real, authenticated UI for this contract, replacing the old unauthenticated `/ingest-test` harness. It implements `empty`/`ready` → `processing` → `successfully_parsed`/`partially_parsed`/`needs_clarification`/`unsupported_format`/`failed` — 5 of the 9 states above (see `lib/ingestion/result-state.ts` for the exact classification rules). `needs_clarification` here means "every extracted item in this run came back `confidence: unclear`" — it's a whole-run result state, not the catch-all `Uncategorized`-with-manual-edit UI this table originally described; a single unclear item within an otherwise fine batch is still shown, just flagged inline, not hidden. `duplicate_input` is still not built — it needs the dedup engine described in `docs/data-model.md` §5, which doesn't exist yet.

The underlying pipeline (`lib/ingestion/ingest.ts`) is unchanged from the original scaffolding pass; only the UI wrapping it moved from a bare dev harness to a real page.
