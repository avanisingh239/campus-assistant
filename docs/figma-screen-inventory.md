...
> **As built:** this screen and the three below it (2.3-2.5) are all implemented as one consolidated route, `/admin/dashboard`, adapting to the signed-in admin's own scope rather than four separate routes — same supersession pattern as §1.4's ingestion drawer note. The three other routes below (`/admin/submit/class`, `/admin/submit/society`, `/admin/history`) no longer exist in the codebase at all — deleted, along with their admin-nav links, once a follow-up pass found them reduced to dead placeholder pages nothing linked to. Course Code, Faculty/Teacher name, and Venue/Room (named in 2.3/2.4's field lists below) aren't collected — no matching `announcements` column exists for any of the three. Active notice counters (2.2) aren't built. See CLAUDE.md's §Admin Dashboard for the exact scope this pass covered.
* **Main Components:**
