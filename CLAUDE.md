...
**`middleware.ts`'s own import of `lib/supabase/middleware.ts` uses a relative path (`./lib/supabase/middleware`), not the `@/*` alias every other file in this repo uses.** Next.js bundles middleware separately from normal pages, in a more restrictive runtime that doesn't rewrite tsconfig's path aliases the same way — this built and ran fine locally (`next dev`, and even `next build`) but crashed in Vercel's production runtime with `Error [ERR_MODULE_NOT_FOUND]: Cannot find package '@/lib'`, since the alias was never resolved to a real path in the deployed middleware bundle and Node tried to load it as an npm package. This is specific to `middleware.ts`'s own import chain — `lib/supabase/middleware.ts` itself only imports `@supabase/ssr`/`next/server` (no further `@/*` imports to fix), and no other file in the app needs this treatment; don't "fix" aliases anywhere else on the strength of this one bug. Verified past `next build` with an actual `next build && next start` run (not just `next dev`, which doesn't reproduce this class of bug) — `/student/dashboard`, `/admin/dashboard`, `/login`, `/`, and `/ingest-test` all resolved correctly with no `ERR_MODULE_NOT_FOUND`.
